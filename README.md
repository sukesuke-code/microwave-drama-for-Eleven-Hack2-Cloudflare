...
### Hackathon alignment (Cloudflare + ElevenLabs)
- Build creative, demo-worthy UX on Cloudflare + ElevenLabs.
- Prefer deep use of Workers, Durable Objects, Workers AI, and edge-native orchestration.
- Ship a high-quality viral-style demo video showing real end-user flow and delight moments.
- Push infra limits responsibly: low-latency paths, robust fallback, and reproducible deployment.
...
### Viral demo strategy (for submission-quality video)
- Show **before/after**: boring microwave wait vs narrated cinematic experience.
- Capture 3 fast scenes: settings selection → countdown drama peak → replay/share loop.
- Keep total demo under ~60–90 seconds with hard cuts and subtitles.
- Include one reliability moment (fallback/recovery still produces a fun result).
...
### Hackathon要件との整合
- Cloudflare + ElevenLabs を使った創造的体験を実装。
- Workers / Durable Objects / Workers AI の活用を強める設計方針。
- エンドユーザー価値が伝わる高品質バイラル動画提出を前提。
- 低遅延・フォールバック・再現可能デプロイを重視してインフラ限界に挑む。
...
### 提出用バイラル動画の構成案
- **Before/After**（退屈な待ち時間→ドラマ体験）を冒頭で即提示。
- 3カット構成：設定→実況ピーク→リプレイ/共有。
- 60〜90秒でテンポよく、字幕付きで無音視聴にも対応。
- 障害時フォールバックでも体験が成立することを1カットで示す。
